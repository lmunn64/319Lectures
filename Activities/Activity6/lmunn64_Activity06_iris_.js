...
function split_and_matrix(matrix){
    ...
    console.log(splitRowByRow);

    for(let element of splitRowByRow){
        let splitElementInRow = element.split(",");
        matrix.push(splitElementInRow);
    }
    console.log(matrix);
}
...
function compute_averages(){
    ...
    setosa_sepal_Length = (setosa_sepal_Width / setosa_count).toFixed(2);
    setosa_sepal_Length = (setosa_petal_Length / setosa_count).toFixed(2);
    setosa_sepal_Length = (setosa_petal_Length / setosa_count).toFixed(2);

    versicolor_sepal_Length = (versicolor_sepal_Length / versicolor_count).toFixed(2);
    versicolor_sepal_Length = (versicolor_sepal_Width / versicolor_count).toFixed(2);
    versicolor_sepal_Length = (versicolor_petal_Length / versicolor_count).toFixed(2);
    versicolor_sepal_Length = (versicolor_petal_Length / versicolor_count).toFixed(2);
    
    virginica_sepal_Length = (virginica_sepal_Length / virginica_count).toFixed(2);
    virginica_sepal_Length = (virginica_sepal_Width / virginica_count).toFixed(2);
    virginica_sepal_Length = (virginica_petal_Length / virginica_count).toFixed(2);
    virginica_sepal_Length = (virginica_petal_Length / virginica_count).toFixed(2);
}
...
// Call function: split the data set and create a matrix
let matrix= [];
split_and_matrix(matrix);  // <- function to construct matrix

// Call function: read matrix, count and sum per class
read_matrix(matrix);      // <- function to traverse matrix per class

// Call function: compute averages
...
const iristable = [
    ["Setosa",setosa_count,setosa_sepal_Length,setosa_sepal_Width,setosa_petal_Length,setosa_petal_Width],
    ["Versicolor",versicolor_count,versicolor_sepal_Length,versicolor_sepal_Width,versicolor_petal_Length,versicolor_petal_Width],
    ["Virginica",virginica_count,virginica_sepal_Length,virginica_sepal_Width,virginica_petal_Length,virginica_petal_Width]
]
